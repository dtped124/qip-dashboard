...
STRICT_INDICATOR_SCHEMA: dict[str, dict] = {
    ...
    "HA01-03": {
        "kind": "rate",
        "names": [
            "(季)急性病床住院案件住院日數超過三十日比率(%)",
            "(季)急性病床住院案件住院日數超過三十日比率(%)(含PAC)",  # 115 竹東：拆成 含/不含PAC，含PAC 為延續歷史數列之權威列
        ],
        "numerator_names": [
            "分母案件中住院超過 30 日的案件數",
            "分母案件中住院超過 30 日的案件數(含PAC)",  # 115 竹東
        ],
        "denominator_names": ["出院案件數（僅急性床案件，不含急慢性床混合案件）"],
    },
    # 竹東專屬（115 起拆分）：不含PAC 版本。114 分頁另有一個代碼仍標 HA01-03
    # 的舊不含PAC 區塊，會被 parser 的 emitted_codes 防護略過，不會誤入。
    "HA01-03-01": {
        "kind": "rate",
        "names": ["(季)急性病床住院案件住院日數超過三十日比率(%)(不含PAC)"],
        "numerator_names": ["分母案件中住院超過 30 日的案件數(排除PAC)"],
        "denominator_names": ["出院案件數（僅急性床案件，不含急慢性床混合案件）"],
    },
...
# ── 數列縫接規則 ────────────────────────────────────────────────
# 115 年起竹東把 HA01-03 拆成 含PAC(沿用 HA01-03)/不含PAC(HA01-03-01)。
# 拆分前(≤113)只有單一數列 → HA01-03-01 的歷史段直接沿用 HA01-03 竹東
# 的資料點(value/n/d 原樣複製)。parser 於每次解析後自動實體化，
# 全歷史重匯時自我修復；來源檔 ≤113 不存在 HA01-03-01 資料列，
# 故縫接不會覆蓋任何真實解析值。114 起雙數列並存，不縫接。
SERIES_STITCH_RULES: dict[str, dict] = {
    "HA01-03-01": {"from_code": "HA01-03", "campus": "竹東", "up_to_year": 113},
}
