    ...
    if (value > ucl) {
      if (direction === 'lower' || direction === 'monitor') {
        anomalies.push({
          ...
          message: `超出 3σ 管制上限 (失控, UCL=${ucl.toFixed(2)})`,
          value,
          referenceValue: ucl,
          year: dp.year,
          month: dp.month,
    ...
    if (value < lcl && lcl > 0) {
      if (direction === 'higher' || direction === 'monitor') {
        anomalies.push({
          ...
          severity: 'alert',
          direction: 'unfavorable',
          message: `低於 3σ 管制下限 (失控, LCL=${lcl.toFixed(2)})`,
          value,
          referenceValue: lcl,
          year: dp.year,
          month: dp.month,
  ...
  for (let i = 2; i < points.length; i++) {
    ...
    if (aboveCount >= 2 && (direction === 'lower' || direction === 'monitor')) {
      ...
      anomalies.push({
        ...
        severity: 'warning',
        direction: 'unfavorable',
        message: `連續 3 點中 ${aboveCount} 點超出 2σ 警戒線上方 (快失控)`,
        value: values[2],
    ...
    if (belowCount >= 2 && (direction === 'higher' || direction === 'monitor')) {
      ...
      anomalies.push({
        ...
        severity: 'warning',
        direction: 'unfavorable',
        message: `連續 3 點中 ${belowCount} 點低於 2σ 警戒線下方 (快失控)`,
        value: values[2],
